import unreal
import warnings


def setup_menu(data):
    parent_menu_name = data.get('parent', "LevelEditor.MainMenu")
    unreal_menus = unreal.ToolMenus.get()
    parent_menu = unreal_menus.find_menu(parent_menu_name)
    if not parent_menu:
        warnings.warn(f"Parent menu '{parent_menu_name}' not found, couldn't setup menu")

    menu_items = _setup_menu_items(parent_menu, data.get('items'))
    unreal_menus.refresh_all_widgets()
    return menu_items
    # TODO decide to return success/failure, or the menu object


def _setup_menu_items(parent_menu, items: list):
    # go over all items in the menu and add them to the menu
    # if there are subitems, call self on data recursively
    created = []
    for item in items:
        label = item.get('label')
        name = item.get('name', label)
        command = item.get('command', None)
        tooltip = item.get('tooltip', "")
        if command:
            entry = add_to_menu(parent_menu, label, command)
            created.append(entry)
        else:  # submenu
            items = item.get('items', [])
            sub_menu = add_sub_menu(parent_menu, label, tooltip)
            _setup_menu_items(sub_menu, items)
            created.append(sub_menu)
    return created  # only return top items, not submenus


def add_to_menu(script_menu, label: str, command: str, tooltip="", icon_name="") -> unreal.ToolMenuEntry:
    """add a menu item to the script menu"""
    entry = unreal.ToolMenuEntry(
        # name="Python.MyCoolTool",  # this needs to be unique! if not set, it's autogenerated
        type=unreal.MultiBlockType.MENU_ENTRY,
        insert_position=unreal.ToolMenuInsert("", unreal.ToolMenuInsertType.FIRST),
    )
    if label:
        entry.set_label(label)
    if command:
        entry.set_string_command(
            type=unreal.ToolMenuStringCommandType.PYTHON,
            string=command,
            custom_type=unreal.Name("_placeholder_"),
        )  # hack: unsure what custom_type does, but it's needed
    if tooltip:
        entry.set_tool_tip(tooltip)
    if icon_name:
        entry.set_icon(icon_name)  # naive implementation todo improve

    script_menu.add_menu_entry("Scripts", entry)  # always returns None
    return entry


def add_sub_menu(script_menu, label: str, tool_tip="") -> unreal.ToolMenu:
    """add a submenu to the script menu"""
    return script_menu.add_sub_menu(
        owner=script_menu.menu_name,
        section_name="PythonTools",
        name=label,  # todo check if needs to be uniqye like in add_to_menu
        label=label,  # todo add label support
        tool_tip=tool_tip,
    )


def add_separator(script_menu) -> None:  # todo not working yet
    """add a separator to the script menu"""
    # see https://docs.unrealengine.com/4.27/en-US/PythonAPI/class/ToolMenu.html
    # todo what is diff with dynamic section?
    return script_menu.add_section(section_name="Test", label="World")


def teardown():
    """remove from menu"""
    raise NotImplementedError("not yet implemented")
