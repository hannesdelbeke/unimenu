...
class MenuNodeUnreal(MenuNodeAbstract):
    ...
    def _setup_menu_item(self, parent_app_node: unreal.ToolMenuEntry = None, kwargs: "dict" = None) -> unreal.ToolMenuEntry:
        """
        add a menu item to the script menu

        parent_app_node: the Unreal parent menu to parent the new menu to
        kwargs: kwargs for unreal.ToolMenuEntry
        """
        if not self.check_unique_name():
            raise Exception(f"Menu item '{self.label}' already exists, stopping menu item setup")

        kwargs = kwargs or {}
        kwargs.setdefault("type", unreal.MultiBlockType.MENU_ENTRY)
        kwargs.setdefault("insert_position", unreal.ToolMenuInsert("", unreal.ToolMenuInsertType.FIRST))

        # name kwarg needs to be unique! if not set, it's autogenerated
        entry = unreal.ToolMenuEntry(name=self.id, **kwargs)
    ...
    def _setup_separator(self, parent_app_node: unreal.ToolMenuEntry = None, kwargs: "dict" = None) -> None:
        ...
        return parent_app_node.add_section(section_name=self.label + "_section", label=self.label + "_label", **kwargs)
